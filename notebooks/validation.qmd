...
matches <- inner_join(ics209plus_dedup, ours_unnest, by = c('INCIDENT_ID' = 'ics_id')) 

ggplot(matches) + 
	geom_point(aes(x = FINAL_ACRES*0.00404686, y = wildfire_area), alpha = .25) + 
	geom_abline(slope = 1, linetype = 'dashed', color = '#88A0DC') + 
	annotate('label', x = 1500, y = 1500, hjust = 0, size = 2, label = '1-1 Line', color = '#88A0DC') + 
	scale_x_continuous(labels = ~ scales::comma(.x, accuracy = .01), trans = 'log') + 
	scale_y_continuous(labels = ~ scales::comma(.x, accuracy = .01), trans = 'log') + 
	xlab("ICS-209-PLUS area (sq. km)") + 
	ylab("WFBZ Disaster area (sq.km)") + 
	coord_equal()
```
