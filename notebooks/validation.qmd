...
ours <- read_sf('../wfbz.geojson') %>% 
	filter(wildfire_year <= 2020)
ics209plus <- read_csv('../data/reference/ics209-plus-wf_incidents_1999to2020.csv') %>% filter(START_YEAR > 1999)
...
ggsave("../figures/validation_n_disasters.pdf", p,
...
```{r}
compare %>% 
	pivot_wider(names_from = dataset, values_from = matches('N')) %>%
	summarize(
		`Mean Diff (WFBZ - ICS-209-PLUS)` = mean(`Wildfire burn zone disasters, N_WFBZ Disaster` - `Wildfire burn zone disasters, N_ICS-209-PLUS`),
		`SD Diff (WFBZ - ICS-209-PLUS)` = sd(`Wildfire burn zone disasters, N_WFBZ Disaster` - `Wildfire burn zone disasters, N_ICS-209-PLUS`),
		`Mean Abs Diff (WFBZ - ICS-209-PLUS)` = mean(abs(`Wildfire burn zone disasters, N_WFBZ Disaster` - `Wildfire burn zone disasters, N_ICS-209-PLUS`)),
		`SD Abs Diff (WFBZ - ICS-209-PLUS)` = sd(abs(`Wildfire burn zone disasters, N_WFBZ Disaster` - `Wildfire burn zone disasters, N_ICS-209-PLUS`))
	) %>%
...
ggsave("../figures/validation_n_fatalities.pdf", p,
       dpi=300, height=10, width=10, units="in")
p
```

Stats for 2014 - 2020 only: 
...
compare %>% 
	...
	pivot_wider(names_from = dataset, values_from = matches('N')) %>%
	summarize(
		`Mean Diff (WFBZ - ICS-209-PLUS)` = mean(`Civilian fatalities, N_WFBZ Disaster` - `Civilian fatalities, N_ICS-209-PLUS`),
		`SD Diff (WFBZ - ICS-209-PLUS)` = sd(`Civilian fatalities, N_WFBZ Disaster` - `Civilian fatalities, N_ICS-209-PLUS`),
		`Mean Abs Diff (WFBZ - ICS-209-PLUS)` = mean(abs(`Civilian fatalities, N_WFBZ Disaster` - `Civilian fatalities, N_ICS-209-PLUS`)),
		`SD Abs Diff (WFBZ - ICS-209-PLUS)` = sd(abs(`Civilian fatalities, N_WFBZ Disaster` - `Civilian fatalities, N_ICS-209-PLUS`))
	) %>%
	pivot_longer(everything())
```
...
ggsave("../figures/validation_n_structures.pdf", p,
       dpi=300, height=7.5, width=10, units="in")
p
```
...
compare %>% 
	pivot_wider(names_from = dataset, values_from = matches('N')) %>%
	summarize(
		`Mean Diff (WFBZ - ICS-209-PLUS)` = mean(`Structures destroyed, N_WFBZ Disaster` - `Structures destroyed, N_ICS-209-PLUS`),
		`SD Diff (WFBZ - ICS-209-PLUS)` = sd(`Structures destroyed, N_WFBZ Disaster` - `Structures destroyed, N_ICS-209-PLUS`),
		`Mean Abs Diff (WFBZ - ICS-209-PLUS)` = mean(abs(`Structures destroyed, N_WFBZ Disaster` - `Structures destroyed, N_ICS-209-PLUS`)),
		`SD Abs Diff (WFBZ - ICS-209-PLUS)` = sd(abs(`Structures destroyed, N_WFBZ Disaster` - `Structures destroyed, N_ICS-209-PLUS`))
	) %>%
	pivot_longer(everything())
```
## Direct Comparisons
...
ggsave("../figures/validation_areas.pdf", p,
       dpi=300, height=7.5, width=10, units="in")
p
```
