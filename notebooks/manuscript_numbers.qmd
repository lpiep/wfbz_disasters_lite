...
```{r}
final <- final %>% filter(wildfire_community_intersect)
nrow(final) 
```
...
max pop den summary
```{r}
summary(final$wildfire_max_pop_den)
```

```{r}
sd(final$wildfire_max_pop_den)
```

cost

```{r}
sum(is.na(final$wildfire_cost))

```

```{r}
summary(final$wildfire_cost[final$wildfire_cost > 0]) 
```

```{r}
sd(final$wildfire_cost[final$wildfire_cost > 0] , na.rm = TRUE) 
```
...
```{r}
sum(is.na(final$wildfire_civil_evacuation))
```

```{r}
summary(final$wildfire_civil_evacuation[final$wildfire_civil_evacuation > 0]) 
```

```{r}
sd(final$wildfire_civil_evacuation[final$wildfire_civil_evacuation > 0] , na.rm = TRUE) 
```
